...
- **`HSA_CACHE_VERSION` bumped `hsa_v1`→`hsa_v2`** when `getHsa` rows gained `needsReview`, `hsa_v2`→`hsa_v3` for the `planYear` block, then **`hsa_v3`→`hsa_v4`** for `planYear.contribRows`. Same rule as the `cfg()`/`nw` trap above: any change to a cached payload's shape needs the version bump or stale reads persist up to the TTL.
